...
const andoverJsonPath = path.resolve('prisma/data/vinyl/andover.json');
let andoverData = JSON.parse(fs.readFileSync(andoverJsonPath, 'utf-8'));

function getSlug(name) {
  let cleanName = name.replace(/ANDOVER/i, '').trim();
  cleanName = cleanName.replace(/®/g, '').trim();
  return cleanName.toLowerCase().replace(/\s+/g, '-');
}

async function scrapeProduct(product) {
  if (product.name.includes("BAYHILL BLONDE")) return product; // Already done
  
  const slug = getSlug(product.name);
  const url = `https://www.msisurfaces.com/luxury-vinyl-planks/andover/${slug}/`;
...
async function main() {
  const updatedData = [];
  for (const product of andoverData) {
  ...
  fs.writeFileSync(andoverJsonPath, JSON.stringify(updatedData, null, 2), 'utf-8');
  console.log('Finished updating andover.json');
}
